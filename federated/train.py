...
def main():
    ...
    print('rank:{0}/{1}'.format(config["rank"], size))
    ...
    if dataset == 'mnist':
        num_classes = 10 
        meta_data = np.load('./data/mnist_data.npz')
        train_data, train_labels, x_test, y_test = meta_data['train_data'], meta_data['train_labels'], meta_data['x_test'], meta_data['y_test']
        ...
        num_classes = 10
        meta_data = np.load('./data/cifar10_data.npz')
        train_data, train_labels, x_test, y_test = meta_data['train_data'], meta_data['train_labels'], meta_data['x_test'], meta_data['y_test']
        ...
        num_classes = 20 
        meta_data = np.load('./data/cifar100_data.npz')
        train_data, train_labels, x_test, y_test = meta_data['train_data'], meta_data['train_labels'], meta_data['x_test'], meta_data['y_test']
    ...
    trainloader = torch.utils.data.DataLoader(trainSet, config['num_tasks'], shuffle=True, num_workers=2, pin_memory=True)
    ...
    configs = '[%s]Train_%s_N_%s_KS_%s_KQ_%s_%s_wd_%s_ulr_%s_mlr_%s_ftlr_%s_ftSP_%s_B_%s_IMG_%s_CE_%s_H_%s_Beta_%s_TS_%s_E_%s_GPU_%s_S_%d_C%d'%(datetime_now, dataset, n_way, k_shot, k_query, config['model_name'], config['weight_decay'], config['update_lr'], config['meta_lr'], eta_ft, update_step_test, config['local_batchsize'], image_size, config['optimizer'], config['H'], config['beta'], config['num_tasks'], config['total_iters'], size, config['seed'], num_classes)
    ...
    for epoch in range(n_epochs):
        ...
        for _, (x_spt, y_spt, x_qry, y_qry, uid) in enumerate(trainloader):
            ...
            # if args.local_rank == 0 and rank==0:
            ...
            if step % 50 == 0 and rank==0:
                    ...
                    for i in range(1): 
                        ...
                        for k in range(update_step_test):
                            ...
                            with torch.no_grad():
                                ...
                                test_acc = np.sum(y_qry[i].cpu().numpy() == pred_q.cpu().numpy()) / len(y_qry[i].cpu().numpy())
                ...
                df = pd.DataFrame(data={'train_acc_H_%s_b_%s_k_%s'%(config['H'], config['beta'], config['num_tasks']):train_acc_list, 'train_acc_H_%s_b_%s_k_%s'%(config['H'], config['beta'], config['num_tasks']):train_acc_list, 'val_acc_H_%s_b_%s_k_%s'%(config['H'], config['beta'], config['num_tasks']) :test_acc_list})
