...
class CIFAR10_MAML(Dataset):
    def __init__(self, train_X, train_Y, mode, N, a, batchsz, n_way, k_shot, k_query, flatten_image=True, image_size=32, num_classes=10, size=3, gpu_rank=0):
        """
    ...
    def create_batch(self, batchsz):
        ...
        for b in range(batchsz):  # for each batch
            ...
            if self.mode == 'train':
                ...
                selected_cls = np.random.choice(self.user_pools[self.gpu_rank], self.n_way, False) 
            else:
...
if __name__ == '__main__':
    ...
    # cifa10 = CIFAR10_MAML(train_data, train_labels, mode='train', N=50, a=68, batchsz=1000, n_way=5, k_shot=5, k_query=10, image_size=32, gpu_rank=0)
    # trainloader = torch.utils.data.DataLoader(cifa10, 5, shuffle=True, num_workers=0, pin_memory=True)
    
    cifa10_test = CIFAR10_MAML(test_data, test_labels, mode='test', N=50, a=34, batchsz=10, n_way=1, k_shot=5, k_query=10, image_size=32, gpu_rank=0)
    testloader = torch.utils.data.DataLoader(cifa10_test, 50, shuffle=False, num_workers=0, pin_memory=True)
